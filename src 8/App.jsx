...
export default function App() {
  ...
  // Autoplay: tries to play, if browser blocks it, user clicks play manually
  ...
  // Next: since it's a single video we just restart it
  const handleNext = () => {
      ...
      <section id="hero" className="hero">
        ...
        >
          ...
          <div className="bar-center">
            {/* Hidden player — autoplay on load */}
            ...
            <div className="bar-controls">
              ...
              <button className="bar-btn" onClick={handleNext} title="Replay">↺</button>
      ...
      <section id="memories" className="memories-section">
        <FadeIn>
          <div className="memories-header">
            ...
            <p className="memories-sub">{ALL_PHOTOS.length} photos & videos · showing {PREVIEW_COUNT} preview</p>
          </div>
        </FadeIn>
        <FadeIn delay={0.15}>
          <div className="memories-strip">
            {previewPhotos.map((src, i) => {
              const isVideo = src.endsWith('.mp4');
              return (
                <div key={i} className="memory-item" onClick={() => openLightbox(src)}>
                  {isVideo ? (
                    <video src={src} className="memory-img" muted playsInline />
                  ) : (
                    <img src={src} alt={`Memory ${i + 1}`} className="memory-img" />
                  )}
                  <div className="memory-overlay">{isVideo ? '▶ Play' : 'View'}</div>
                </div>
              );
            })}
            {ALL_PHOTOS.length > PREVIEW_COUNT && (
              <div className="memory-more" onClick={handleDownloadZip}>
                ...
                <span className="memory-more-label">more<br />in download</span>
              </div>
            )}
          </div>
        </FadeIn>
        <FadeIn delay={0.3}>
          ...
          <p className="download-note">Downloads all {ALL_PHOTOS.length} photos & videos as a zip file</p>
        </FadeIn>
      </section>
